...
Real
NavierStokesBase::calcHorizontalAvg()
{
// Compute the horizontal average velocity at a z-plane of 90 m
     ...
     MultiFab&   S_new    = amr_level.get_old_data(State_Type);
     ...
     for (MFIter mfi(S_new,true); mfi.isValid(); ++mfi)
      {
        ...
        Real        s;
        const Real* dat = Sfab.dataPtr();
        const int*  dlo = Sfab.loVect();
        const int*  dhi = Sfab.hiVect();
        const int*  lo  = bx.loVect();
        const int*  hi  = bx.hiVect();
        RealBox    gridloc = RealBox(bx,geom.CellSize(),geom.ProbLo());

        calc_horizontal_avg(dat,ARLIM(dlo),ARLIM(dhi),ARLIM(lo),ARLIM(hi),
                dx,gridloc.lo(),&s);
        hori_avg += s;
        //std::cout << "Individual horizontal avg" << s << "\n";

    }

    ParallelDescriptor::ReduceRealSum(hori_avg); 
    hori_avg = hori_avg/(128*128);
    if (ParallelDescriptor::IOProcessor())
    {
    std::cout << "Horizontal avg" << hori_avg << "\n";
    }
    // Horizontal averaging ends
    return hori_avg;
}
			    ...
			    const Real	     hori_avg)
{
    ...
    FORT_MAKEFORCE (&time,
		    ...
		    gridloc.lo(),
		    gridloc.hi(),
		    &grav,&scomp,&ncomp,&nscal,&getForceVerbose,&hori_avg);
