...
subroutine calc_horizontal_avg(rho,DIMS(rho),DIMS(grid),delta,xlo,mass)&
                          bind(C,name="calc_horizontal_avg")

       implicit none

       integer DIMDEC(rho)
       integer DIMDEC(grid)
       REAL_T  mass, delta(SDIM), xlo(SDIM)
       REAL_T  rho(DIMV(rho),6)
       ...
       do k = ARG_L3(grid), ARG_H3(grid)
	  z = xlo(3) + delta(3)*(float(k-ARG_L3(grid)) + 0.5d0)
          do j = ARG_L2(grid), ARG_H2(grid)
	     y = xlo(2) + delta(2)*(float(j-ARG_L2(grid)) + 0.5d0)
             do i = ARG_L1(grid), ARG_H1(grid)
    	        x = xlo(1) + delta(1)*(float(i-ARG_L1(grid)) + 0.5d0)
		if(x.ge.0.0d0.and.y.ge.0.0d0)then
		  if(z.ge.78.125d0.and.z.le.93.75d0)then
		  !if(z.ge.365.0d0.and.z.le.380.0d0)then
		  !if(z.ge.62.5d0.and.z.le.93.25d0)then
                    mass = mass + rho(i,j,k,1)
		  endif
		endif
             end do
          end do
       ...
       end subroutine calc_horizontal_avg
