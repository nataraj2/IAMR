...
!c        node wide in direction of face.
	...
	rho=-0.0034d0*(rho-300.0d0)
	do k = ARG_H3(phi)-1,ARG_L3(phi),-1
	   !rho(:,:,k) = 300.0d0+10.0d0*(float(k-ARG_L3(phi))+0.5)*dx(3)
	enddo
        ...
        do j = ARG_L2(phi)+1,ARG_H2(phi)-1
          rhog = zero
          do k = ARG_H3(phi)-1,ARG_L3(phi),-1
		!print*, rho(i,j,k), rho(i-1,j,k), rho(i-2,j,k),rho(i-2,j-1,k), 300.0d0+10.0d0*(float(k-ARG_L3(phi))+0.5)*dx(3)
		!stop
