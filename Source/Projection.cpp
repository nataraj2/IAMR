...
#pragma omp parallel
	...
	Vector<std::unique_ptr<MultiFab> > temp(maxlev);
	const int       nghost = 1;
        const BoxArray& grids  = LevelData[level]->boxArray();
        const DistributionMapping& dmap = LevelData[level]->DistributionMap();
        temp[level].reset(new MultiFab(grids,dmap,1,nghost));

        AmrLevel& amr_level = parent->getLevel(level);

	MultiFab& S_new = amr_level.get_new_data(State_Type);

        S_new.setBndry(BogusValue,Density+2,1);
        ...
        S_new.FillBoundary(Density+2,1,geom.periodicity());
        for (MFIter mfi(S_new); mfi.isValid(); ++mfi)
        {
            amr_level.setPhysBoundaryValues(S_new[mfi],State_Type,curr_time,Density+2,Density+2,1);
        }

	MultiFab::Copy(*temp[level],LevelData[level]->get_new_data(State_Type),Density+2,0,1,1);
        set_outflow_bcs(LEVEL_PROJ,phi,Vel_ML,Divu_ML,amrex::GetVecOfPtrs(temp),level,level,have_divu);
...
Projection::initialPressureProject (int  c_lev)
{
    ...
    for (lev = c_lev; lev <= f_lev; lev++) 
    {
        ...
        const DistributionMapping& dmap = LevelData[lev]->DistributionMap();
        sig[lev].reset(new MultiFab(grids,dmap,1,nghost));
        temp[lev].reset(new MultiFab(grids,dmap,1,nghost));
    ...
    for (lev = c_lev; lev <= f_lev; lev++) {
        ...
        //vel[lev]->setVal(300.0*gravity, BL_SPACEDIM-1, 1            , 1);
	MultiFab::Copy(*vel[lev],LevelData[lev]->get_new_data(State_Type),Density+2,BL_SPACEDIM-1,1,1);
	vel[lev]->plus(-300.0,BL_SPACEDIM-1,1,1);
	vel[lev]->mult(-0.0034*gravity,BL_SPACEDIM-1,1,1);
    }
