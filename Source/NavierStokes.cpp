...
#pragma omp parallel
    ...
    hori_avg = calcHorizontalAvg();
    //std::cout << "HERE 3......." << hori_avg << "\n";

    {
      ...
      for (MFIter U_mfi(Umf,true); U_mfi.isValid(); ++U_mfi)
      {
        ...
        getForce(tforces,bx,1,Xvel,BL_SPACEDIM,prev_time,Ufab,Smf[U_mfi],0,hori_avg);
...
#pragma omp parallel
    ...
    { 
      ...
      for (MFIter S_mfi(Smf,true); S_mfi.isValid(); ++S_mfi)
      {
        ...
        getForce(tforces,bx,nGrowF,fscalar,num_scalars,prev_time,Umf[S_mfi],Smf[S_mfi],0,hori_avg);
                             ...
                             const int  ncycle)
{
    ...
    // Calculate viscosity
    //
    const int nGrow = 2;//visc_cc->nGrow();
	//std::cout << "nGrow is...." << nGrow << "\n";

    if (is_diffusive[Xvel]) 
    {
        if (visc_coef[Xvel] >= 0.0)
        {
		visc_cc->setVal(0.0, 0, 1, visc_cc->nGrow());
		//visc_cc->setVal(0.0, 0, 1, nGrow);
		AmrLevel& amr_level = parent->getLevel(level);
	        MultiFab&   S_new    = amr_level.get_new_data(State_Type);

	        const Real BogusValue = 1.e200;

        	S_new.setBndry(BogusValue,Density+1,nGrow);
	        ...
	        S_new.FillBoundary(Density+1,1,geom.periodicity());
        	for (MFIter mfi(S_new); mfi.isValid(); ++mfi)
       		{
	            amr_level.setPhysBoundaryValues(S_new[mfi],State_Type,curr_time,Density+1,Density+1,nGrow);
        	}

		MultiFab Sborder(grids, dmap, S_new.nComp(), nGrow);
	        FillPatch(*this, Sborder, nGrow, 0.0, State_Type, 0, 6);
    		...
    		{
		        const Box& bx = mfi.tilebox();
		        calculateViscosity((*visc_cc)[mfi],Sborder[mfi],bx,dx);
	        }
	}
        else
        {
...
void NavierStokes::calculateViscosity(FArrayBox& viscfab, FArrayBox& Sfab, const Box& bx, const Real* dx)
{

	const int* s_lo    = Sfab.loVect();
        const int* s_hi    = Sfab.hiVect();
	RealBox    gridloc = RealBox(bx,geom.CellSize(),geom.ProbLo());
	FORT_CALCULATEVISCOSITY(bx.loVect(), bx.hiVect(), ARLIM(viscfab.loVect()), ARLIM(viscfab.hiVect()), viscfab.dataPtr(), Sfab.dataPtr(Xvel),
                       Sfab.dataPtr(BL_SPACEDIM), ARLIM(s_lo), ARLIM(s_hi), dx, gridloc.lo());
